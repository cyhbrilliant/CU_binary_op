...
//check bin
    ...
    dim3 BS_MM(32,1,1);
    dim3 GS_MM(1000,1,1);
    MatrixMulXnor<<<GS_MM,BS_MM,sizeof(int)*MaxBS>>>(a_device_bin , b_device_bin , a_rows , MaxBS , b_cols ,
...
//simulate pytorch param
    ...
    float *b_device;
    float *result_device;
    cudaMalloc((void**)&a_device,sizeof(float) *x * n);
    cudaMalloc((void**)&b_device,sizeof(float) *n * y);
    cudaMalloc((void**)&result_device,sizeof(float) *x * y);
    cudaMemcpy(a_device,a_host,sizeof(float) *x * n,cudaMemcpyHostToDevice);
    cudaMemcpy(b_device,b_host,sizeof(float) *n * y,cudaMemcpyHostToDevice);
...
//run in gpu warp in C code
    MatrixMul_device(a_device,b_device,x,n,y,result_device);
